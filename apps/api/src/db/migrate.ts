  ...
  try {
    ...
    await db.run(sql`
      CREATE TABLE IF NOT EXISTS uploaded_files (
        ...
        status TEXT NOT NULL DEFAULT 'stored' CHECK(status IN ('stored', 'processing', 'processed', 'failed')),
        failure_reason TEXT,
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL
      )
    ...
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS uploaded_file_user_idx ON uploaded_files(user_id)`
    );
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS uploaded_file_status_idx ON uploaded_files(status)`
    );
    await db.run(
      sql`CREATE UNIQUE INDEX IF NOT EXISTS uploaded_file_storage_key_idx ON uploaded_files(storage_key)`
    );

    // File parsed summaries table
    await db.run(sql`
      CREATE TABLE IF NOT EXISTS file_parsed_summaries (
        id TEXT PRIMARY KEY NOT NULL,
        file_id TEXT NOT NULL,
        parser_version TEXT NOT NULL,
        document_type TEXT NOT NULL CHECK(document_type IN ('receipt', 'invoice', 'bank_statement', 'excel_table')),
    ...
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS parsed_summary_file_idx ON file_parsed_summaries(file_id)`
    );
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS parsed_summary_version_idx ON file_parsed_summaries(parser_version)`
    );

    // File imported items table
    await db.run(sql`
    ...
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS imported_item_file_idx ON file_imported_items(file_id)`
    );
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS imported_item_transaction_idx ON file_imported_items(transaction_id)`
    );
    await db.run(
      sql`CREATE UNIQUE INDEX IF NOT EXISTS imported_item_unique_idx ON file_imported_items(file_id, parsed_item_id)`
    );
