...
import { createGoal, updateGoal, deleteGoal, contributeToGoal, type Goal, type GoalType, type GoalStatus } from '@/lib/api';
      ...
      <div className="min-h-screen bg-gray-950 text-white">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 lg:py-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 lg:mb-8">
            <div>
              ...
              <p className="text-sm lg:text-base text-gray-400">Visualiza tu progreso hacia tus sueños</p>
            </div>
            <button
        ...
        {goals.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <h2 className="text-2xl font-bold mb-2">¡Empieza a soñar en grande!</h2>
            <p className="text-gray-400 text-center max-w-md mb-8">
              Define tus metas financieras y rastrea tu progreso.
              Ya sea ahorrar para un viaje, un carro, o tu fondo de emergencia.
            </p>
            <div className="flex gap-4">
              <button
                onClick={() => setShowAddModal(true)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredGoals.map((goal) => (
                ...
                >
                  ...
                  <div className={`relative h-32 bg-gradient-to-br ${getProgressColor(goal.progressPercent, goal.onTrack)} p-6`}>
                    <div className="absolute inset-0 bg-black/30" />
                    <div className="relative flex items-start justify-between">
                      <span className="text-5xl">{goal.emoji || GOAL_TYPE_LABELS[goal.goalType]?.emoji || '🎯'}</span>
                      <span className={`px-2 py-1 text-xs rounded ${STATUS_LABELS[goal.status]?.color} backdrop-blur-sm`}>
                        {STATUS_LABELS[goal.status]?.label}
                      </span>
                    </div>
                    <div className="absolute bottom-4 right-4 text-right">
                      <p className="text-3xl font-bold text-white">{Math.round(goal.progressPercent)}%</p>
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-sm text-gray-400 mb-4">{GOAL_TYPE_LABELS[goal.goalType]?.label}</p>
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-sm mb-2">
                        <span className="text-green-400">{formatCurrency(goal.currentAmountCents)}</span>
                        <span className="text-gray-400">{formatCurrency(goal.targetAmountCents)}</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm mb-4">
                      <div>
                        ...
                        <p className={`font-medium ${goal.targetDate && new Date(goal.targetDate) < new Date() ? 'text-red-400' : 'text-white'}`}>
                          {getDaysRemaining(goal.targetDate)}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-500">Aporte mensual sugerido</p>
                        <p className="font-medium text-cyan-400">
                          {goal.recommendedMonthlyCents ? formatCurrency(goal.recommendedMonthlyCents) : 'N/A'}
                        </p>
                      </div>
                    </div>
                    ...
                    {/* Status indicator */}
                    {goal.status === 'active' && (
                      <div className={`flex items-center gap-2 text-sm mb-4 ${goal.onTrack ? 'text-green-400' : 'text-orange-400'}`}>
                        <span className="relative flex h-2 w-2">
                          <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${goal.onTrack ? 'bg-green-400' : 'bg-orange-400'} opacity-75`}></span>
                          <span className={`relative inline-flex rounded-full h-2 w-2 ${goal.onTrack ? 'bg-green-500' : 'bg-orange-500'}`}></span>
                        </span>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-800">
              ...
              <p className="text-sm text-gray-400">Define tu próximo objetivo financiero</p>
            </div>
            <form onSubmit={handleAddGoal} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-cyan-500"
                >
                  {Object.entries(GOAL_TYPE_LABELS).map(([key, { label, emoji }]) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Monto Meta</label>
                  <input
                    type="number"
                    value={newGoal.target_amount_cents || ''}
                    onChange={(e) => setNewGoal({ ...newGoal, target_amount_cents: parseFloat(e.target.value) || 0 })}
                    className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-cyan-500"
                    placeholder="50000"
                    step="0.01"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewGoal({ ...newGoal, current_amount_cents: parseFloat(e.target.value) || 0 })}
                    className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:border-cyan-500"
                    placeholder="0"
                    step="0.01"
                  />
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1">Fecha Límite (opcional)</label>
                <input
                  type="date"
          ...
          <div className="bg-gray-900 rounded-2xl max-w-md w-full">
            <div className="p-6 border-b border-gray-800">
              <div className="flex items-center gap-3">
                <span className="text-3xl">{showContributeModal.emoji || GOAL_TYPE_LABELS[showContributeModal.goalType]?.emoji}</span>
                <div>
                  <h2 className="text-xl font-bold">Aportar a Meta</h2>
                  <p className="text-sm text-gray-400">{showContributeModal.name}</p>
                </div>
              </div>
            </div>
            <form onSubmit={handleContribute} className="p-6 space-y-4">
              <div className="bg-gray-800/50 rounded-lg p-4 mb-4">
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="text-white">{Math.round(showContributeModal.progressPercent)}%</span>
                </div>
                <div className="h-2 bg-gray-700 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(showContributeModal.progressPercent, 100)}%` }}
                  />
                </div>
                <div className="flex justify-between text-xs text-gray-500 mt-2">
                  <span>{formatCurrency(showContributeModal.currentAmountCents)}</span>
                  <span>{formatCurrency(showContributeModal.targetAmountCents)}</span>
                </div>
              </div>

              <div>
                <label className="block text-sm text-gray-400 mb-1">Monto a Aportar</label>
                <input
                  type="number"
                  value={contribution.amount}
                  onChange={(e) => setContribution({ amount: e.target.value })}
              ...
              <p className="text-center text-sm text-gray-400">
                Nuevo total: {formatCurrency(showContributeModal.currentAmountCents + (parseFloat(contribution.amount) || 0) * 100)}
              </p>
